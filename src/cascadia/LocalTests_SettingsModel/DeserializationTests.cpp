...
namespace SettingsModelLocalTests
{
    ...
    {
        const auto settingsWithProfiles{ R"(
        {
            "profiles": [
                {
                    "name" : "profile0"
        ...
        const auto settingsWithoutProfiles{ R"(
        {
            "defaultProfile": "{6239a42c-1de4-49a3-80bd-e8fdd045185c}"
        })" };

        const auto settingsWithEmptyProfiles{ R"(
        {
            "profiles": []
        })" };
    ...
    void DeserializationTests::ValidateDefaultProfileExists()
    {
        ...
        const auto goodProfilesSpecifiedByName{ R"(
        {
...

        {
            ...
            const auto settings = winrt::make_self<implementation::CascadiaSettings>(goodProfiles);
        ...
        }
        {
            ...
            const auto settings = winrt::make_self<implementation::CascadiaSettings>(badProfiles);
        ...
        }
        {
            ...
            const auto settings = winrt::make_self<implementation::CascadiaSettings>(goodProfilesSpecifiedByName);
            VERIFY_ARE_EQUAL(static_cast<size_t>(0), settings->Warnings().Size());
            VERIFY_ARE_EQUAL(static_cast<size_t>(2), settings->AllProfiles().Size());
    ...
    void DeserializationTests::ValidateDuplicateProfiles()
    {
        ...
        const auto settings = winrt::make_self<implementation::CascadiaSettings>(veryBadProfiles);
    ...
    void DeserializationTests::ValidateManyWarnings()
    {
        ...
        const Profile profile4 = winrt::make<implementation::Profile>(Utils::GuidFromString(L"{6239a42c-4444-49a3-80bd-e8fdd045185c}"));
        profile4.Name(L"profile4");
        const Profile profile5 = winrt::make<implementation::Profile>(Utils::GuidFromString(L"{6239a42c-4444-49a3-80bd-e8fdd045185c}"));
        ...
        settings->AllProfiles().Append(profile4);
        settings->AllProfiles().Append(profile5);

        VERIFY_ARE_EQUAL(3u, settings->Warnings().Size());
        VERIFY_ARE_EQUAL(SettingsLoadWarnings::DuplicateProfile, settings->Warnings().GetAt(0));
        VERIFY_ARE_EQUAL(SettingsLoadWarnings::MissingDefaultProfile, settings->Warnings().GetAt(1));
        VERIFY_ARE_EQUAL(SettingsLoadWarnings::UnknownColorScheme, settings->Warnings().GetAt(2));
    ...
    {
        const auto settings0String{ R"(
        {
            "alwaysShowTabs": true,
            "initialCols" : 120,
            "initialRows" : 30
        })" };
        const auto settings1String{ R"(
        {
    ...
    {
        const auto userProfiles0String{ R"(
        {
            "profiles": [
                {
                    "name" : "profile0",
        ...
        const auto defaultProfilesString{ R"(
        {
            "profiles": [
                {
                    "name" : "profile2",
        ...
        const auto userProfiles1String{ R"(
        {
            "profiles": [
                {
                    "name" : "profile4",
    ...
    void DeserializationTests::ValidateProfilesGenerateGuids()
    {
        ...
        const auto profile2String{ R"(
        {
            "name" : "profile2",
            "guid" : null
        })" };
        const auto profile3String{ R"(
        {
            "name" : "profile3",
            "guid" : "{00000000-0000-0000-0000-000000000000}"
        })" };
        const auto profile4String{ R"(
        {
            "name" : "profile4",
            "guid" : "{6239a42c-1de4-49a3-80bd-e8fdd045185c}"
        })" };
        const auto profile5String{ R"(
        {
            "name" : "profile2"
        })" };
        ...
        settings->AllProfiles().Append(*profile0);
        settings->AllProfiles().Append(*profile1);
        settings->AllProfiles().Append(*profile2);
        settings->AllProfiles().Append(*profile3);
        settings->AllProfiles().Append(*profile4);
        settings->AllProfiles().Append(*profile5);

        VERIFY_IS_FALSE(settings->AllProfiles().GetAt(0).HasGuid());
        VERIFY_IS_FALSE(settings->AllProfiles().GetAt(1).HasGuid());
        VERIFY_IS_FALSE(settings->AllProfiles().GetAt(2).HasGuid());
        VERIFY_IS_TRUE(settings->AllProfiles().GetAt(3).HasGuid());
        VERIFY_IS_TRUE(settings->AllProfiles().GetAt(4).HasGuid());
        VERIFY_IS_FALSE(settings->AllProfiles().GetAt(5).HasGuid());

        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(0).Guid(), nullGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(1).Guid(), nullGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(2).Guid(), nullGuid);
        VERIFY_ARE_EQUAL(settings->AllProfiles().GetAt(3).Guid(), nullGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(4).Guid(), nullGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(5).Guid(), nullGuid);

        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(0).Guid(), cmdGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(1).Guid(), cmdGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(2).Guid(), cmdGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(3).Guid(), cmdGuid);
        VERIFY_ARE_EQUAL(settings->AllProfiles().GetAt(4).Guid(), cmdGuid);
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(5).Guid(), cmdGuid);

        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(0).Guid(), settings->AllProfiles().GetAt(2).Guid());
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(1).Guid(), settings->AllProfiles().GetAt(2).Guid());
        VERIFY_ARE_EQUAL(settings->AllProfiles().GetAt(2).Guid(), settings->AllProfiles().GetAt(2).Guid());
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(3).Guid(), settings->AllProfiles().GetAt(2).Guid());
        VERIFY_ARE_NOT_EQUAL(settings->AllProfiles().GetAt(4).Guid(), settings->AllProfiles().GetAt(2).Guid());
        VERIFY_ARE_EQUAL(settings->AllProfiles().GetAt(5).Guid(), settings->AllProfiles().GetAt(2).Guid());
    }
    ...
    void DeserializationTests::TestLayeringNameOnlyProfiles()
    {
        ...
        VERIFY_IS_TRUE(settings->AllProfiles().GetAt(1).HasGuid());
        VERIFY_IS_TRUE(settings->AllProfiles().GetAt(2).HasGuid());
        VERIFY_IS_FALSE(settings->AllProfiles().GetAt(3).HasGuid());
        VERIFY_IS_FALSE(settings->AllProfiles().GetAt(4).HasGuid());
        VERIFY_ARE_EQUAL(L"Windows PowerShell", settings->AllProfiles().GetAt(0).Name());
        VERIFY_ARE_EQUAL(L"Command Prompt", settings->AllProfiles().GetAt(1).Name());
        VERIFY_ARE_EQUAL(L"ThisProfileIsGood", settings->AllProfiles().GetAt(2).Name());
        VERIFY_ARE_EQUAL(L"ThisProfileShouldNotLayer", settings->AllProfiles().GetAt(3).Name());
        VERIFY_ARE_EQUAL(L"NeitherShouldThisOne", settings->AllProfiles().GetAt(4).Name());
    }
    ...
    void DeserializationTests::TestHideAllProfiles()
    {
        ...
        {
            // Case 1: Good settings
            const auto settings = winrt::make_self<implementation::CascadiaSettings>(settingsWithProfiles);
            VERIFY_ARE_EQUAL(2u, settings->AllProfiles().Size());
            VERIFY_ARE_EQUAL(1u, settings->ActiveProfiles().Size());
        }
        {
    ...
    void DeserializationTests::TestInvalidColorSchemeName()
    {
        ...
        VERIFY_ARE_EQUAL(L"schemeOne", settings->AllProfiles().GetAt(0).DefaultAppearance().ColorSchemeName());
        VERIFY_ARE_EQUAL(L"Campbell", settings->AllProfiles().GetAt(1).DefaultAppearance().ColorSchemeName());
        VERIFY_ARE_EQUAL(L"Campbell", settings->AllProfiles().GetAt(2).DefaultAppearance().ColorSchemeName());
    }
    ...
    void DeserializationTests::ValidateColorSchemeInCommands()
    {
        ...
        }
        {
            ...
            const auto settings = winrt::make_self<implementation::CascadiaSettings>(settings1String);
        ...
        }
        {
            ...
            const auto settings = winrt::make_self<implementation::CascadiaSettings>(settings2String);
    ...
    void DeserializationTests::TestHelperFunctions()
    {
        ...
        VERIFY_ARE_EQUAL(badGuid, settings->GetProfileByName(badName).Guid());

        const auto prof0{ settings->FindProfile(guid0) };
        const auto prof1{ settings->FindProfile(guid1) };
        const auto prof2{ settings->FindProfile(guid2) };

        const auto badProf{ settings->FindProfile(fakeGuid) };
    ...
    void DeserializationTests::TestDontLayerGuidFromUserDefaults()
    {
        ...
        VERIFY_ARE_EQUAL(guid1, settings->AllProfiles().GetAt(2).Guid());
        VERIFY_IS_FALSE(settings->AllProfiles().GetAt(3).HasGuid());
    }
    ...
    void DeserializationTests::TestLayerUserDefaultsOnDynamics()
    {
        ...
        const auto userProfiles{ R"(
        {
            ...
            "profiles": {
                ...
                "list": [
                    {
                        "name" : "profile0FromUserSettings", // this is AllProfiles().GetAt(0)
                        "guid": "{6239a42c-1111-49a3-80bd-e8fdd045185c}",
                        "source": "Terminal.App.UnitTest.0"
                    },
                    {
                        "name" : "profile1FromUserSettings", // this is AllProfiles().GetAt(2)
                        "guid": "{6239a42c-2222-49a3-80bd-e8fdd045185c}",
                        "source": "Terminal.App.UnitTest.1",
                        "historySize": 4444
                    },
                    {
                        "name" : "profile2FromUserSettings", // this is AllProfiles().GetAt(3)
                        "guid": "{6239a42c-3333-49a3-80bd-e8fdd045185c}",
                        "historySize": 5555
                    }
                ]
        ...
        VERIFY_IS_TRUE(settings->AllProfiles().GetAt(3).Source().empty());

        VERIFY_ARE_EQUAL(L"Terminal.App.UnitTest.0", settings->AllProfiles().GetAt(0).Source());
        VERIFY_ARE_EQUAL(L"Terminal.App.UnitTest.1", settings->AllProfiles().GetAt(1).Source());
        VERIFY_ARE_EQUAL(L"Terminal.App.UnitTest.1", settings->AllProfiles().GetAt(2).Source());

        VERIFY_ARE_EQUAL(guid1, settings->AllProfiles().GetAt(0).Guid());
        VERIFY_ARE_EQUAL(guid1, settings->AllProfiles().GetAt(1).Guid());
        VERIFY_ARE_EQUAL(guid2, settings->AllProfiles().GetAt(2).Guid());

        VERIFY_ARE_EQUAL(L"profile0FromUserSettings", settings->AllProfiles().GetAt(0).Name());
        VERIFY_ARE_EQUAL(L"profile0", settings->AllProfiles().GetAt(1).Name());
        VERIFY_ARE_EQUAL(L"profile1FromUserSettings", settings->AllProfiles().GetAt(2).Name());
        VERIFY_ARE_EQUAL(L"profile2FromUserSettings", settings->AllProfiles().GetAt(3).Name());
        ...
        VERIFY_ARE_EQUAL(1234, settings->AllProfiles().GetAt(0).HistorySize());
        VERIFY_ARE_EQUAL(1234, settings->AllProfiles().GetAt(1).HistorySize());
        VERIFY_ARE_EQUAL(4444, settings->AllProfiles().GetAt(2).HistorySize());
        VERIFY_ARE_EQUAL(5555, settings->AllProfiles().GetAt(3).HistorySize());
    }
    ...
    void DeserializationTests::ValidateKeybindingsWarnings()
    {
        ...
        VERIFY_ARE_EQUAL(2u, settings->GlobalSettings().ActionMap().AvailableActions().Size());
        VERIFY_ARE_EQUAL(1u, settings->GlobalSettings().ActionMap().NameMap().Size());
    ...
    void DeserializationTests::ValidateExecuteCommandlineWarning()
    {
        ...
        VERIFY_IS_NULL(settings->GlobalSettings().ActionMap().GetActionByKeyChord({ VirtualKeyModifiers::Control, static_cast<int32_t>('A'), 0 }));
        VERIFY_IS_NULL(settings->GlobalSettings().ActionMap().GetActionByKeyChord({ VirtualKeyModifiers::Control, static_cast<int32_t>('B'), 0 }));
        VERIFY_IS_NULL(settings->GlobalSettings().ActionMap().GetActionByKeyChord({ VirtualKeyModifiers::Control, static_cast<int32_t>('C'), 0 }));
    ...
    void DeserializationTests::TestCommandsAndKeybindings()
    {
        ...
        VERIFY_ARE_EQUAL(5u, actionMap->AvailableActions().Size());
    ...
    void DeserializationTests::TestCopy()
    {
        ...
        // test actions
        VERIFY_ARE_EQUAL(settings->GlobalSettings().ActionMap().AvailableActions().Size(), copyImpl->GlobalSettings().ActionMap().AvailableActions().Size());
